...
public static class LudumDareJobParser 
{
	[MenuItem("Pre Production/Parse Ludum Dare Jobs")]
	...
	{
		string readPath = string.Format("{0}/Settings/LudumDare-JobStartingStats.csv", Application.dataPath);
	...
	{
		string readPath = string.Format("{0}/Settings/LudumDare-JobGrowthStats.csv", Application.dataPath);
		string[] readText = File.ReadAllLines(readPath);
		for (int i = 1; i < readText.Length; ++i)
